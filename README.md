...
该项目设计了一个通用的电机控制系统，可以接收多种信号源的控制命令（遥控器、串口指令等），并通过不同的驱动方式控制电机运动。系统支持多种电机驱动器，包括直接 PWM 控制、RS485 通信控制和 CAN 通信控制等方式。
...
* **多种驱动方式**：
  ...
  * CAN 总线驱动：
    * 单路 CAN 驱动（`drv_keya.c`）
    * 双路 CAN 驱动，支持 LKBLS481502 驱动器（`drv_keyadouble.c`）
...
* **Core/**: 核心应用代码
  ...
  * **Src/**: 源文件
    ...
    * `channel_parse.c`: 遥控器通道和命令解析功能
    * `drv_real.c`: RS485 协议电机驱动实现
    * `drv_keyadouble.c`: 双路 CAN 总线电机驱动实现
...
* 支持 LKBLS481502 双路电机驱动器
* 通信 ID：TX_ID 0x06000000，RX_ID 0x05800000
* 指令类型：使能电机 (0x01)、失能电机 (0x02)、设置速度 (0x03)
* 支持 A 通道（左）和 B 通道（右）独立控制
...
4. **改变驱动类型**：
   * 修改 `channel_parse.c` 文件中的驱动选择宏定义
   * 重新编译项目
